create table splits as
    select l, (
        select string_agg(case v when '.' then 0 else 1 end, '')::bitstring
...
-- start special case
update splits set arr = arr | (select arr from splits where l=1) where l=2;
select * from splits order by l;

create macro r(a) as (select arr from splits s where s.l=a);

-- create macro e(b, a) as
--     regexp_replace(
--        ( r(b)&r(a) )::varchar, '010', '101')::bit;

create macro e(b, a) as
    ((r(b)&r(a)) << 1) | ((r(b)&r(a)) >> 1);

create macro c(b) as e(b-2,b-1) | (r(b-2) & ~r(b-1));
...
-- select *, bit_count(r(l-1)&r(l-2)) from splits order by l;
select sum(bit_count(r(l-1)&r(l-2)))-1 from splits;


-- create table j as values (1),(7);
-- select bitstring(bitstring_agg(col0,1,99),length(r(1))::int) from j;